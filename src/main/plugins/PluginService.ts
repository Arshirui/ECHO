...
const exampleTemplates: Record<PluginCreateExampleKind, { id: string; name: string; manifest: PluginManifest; script: string; panel?: string }> = {
  ...
  'audio-authenticity': {
    ...
    manifest: {
      ...
      contributes: {
        commands: [
          { id: 'analyze-page', title: 'Analyze audio authenticity' },
          { id: 'analyze-track', title: 'Analyze selected track authenticity' },
        ],
        trackContextMenus: [{ id: 'audio-authenticity', title: '音频可信度', commandId: 'analyze-track', localOnly: true }],
        panels: [{ id: 'main', title: 'Audio Authenticity', path: 'panel.html' }],
      },
    },
    script: [
      'const pageSize = 8;',
      ...
      "  unknown: '暂时无法判断'",
      '};',
      "const verdictTone = (verdict) => ['trusted_lossless', 'trusted_dsd_container'].includes(verdict) ? 'good' : ['likely_lossy_transcode', 'likely_fake_hires', 'dsd_metadata_mismatch'].includes(verdict) ? 'risk' : verdict === 'lossy_source' ? 'warn' : 'neutral';",
      "const sampleRate = (value) => value ? `${value >= 1000000 ? Math.round(value / 10000) / 100 : Math.round(value / 100) / 10} ${value >= 1000000 ? 'MHz' : 'kHz'}` : '--';",
      "const bitrate = (value) => value ? `${Math.round(value / 1000)} kbps` : '--';",
      ...
      "    { label: '采样率', value: sampleRate(report.metrics.sampleRate) },",
      "    { label: '位深', value: report.metrics.bitDepth ? `${Math.round(report.metrics.bitDepth)} bit` : '--' },",
      "    { label: '码率', value: bitrate(report.metrics.bitrate) }",
      '  ],',
      '  evidence: report.evidence,',
      '  notes: report.limitations',
      '});',
      '',
      'const summarize = (track, report) => ({',
      '  id: track.id,',
      '  title: track.title,',
      '  artist: track.artist,',
      '  codec: track.codec,',
      ...
      '});',
      '',
      "echo.commands.register('analyze-page', { title: 'Analyze audio authenticity' }, async (pageInput = 1) => {",
      '  const page = Math.max(1, Math.floor(Number(pageInput) || 1));',
      '  const tracks = await echo.library.getTracks({',
      '    page,',
      '    pageSize,',
      "    fields: ['id', 'title', 'artist', 'codec', 'sampleRate', 'bitDepth', 'bitrate']",
      '  });',
      '  const reports = [];',
      '  for (const track of tracks.items) {',
      '    if (!track.id) continue;',
      '    try {',
      '      const report = await echo.audio.analyzeTrack(track.id);',
      '      reports.push(summarize(track, report));',
      '    } catch (error) {',
      '      reports.push({ id: track.id, title: track.title, error: error instanceof Error ? error.message : String(error) });',
      '    }',
      '  }',
      '  const result = { page: tracks.page, pageSize: tracks.pageSize, total: tracks.total, hasMore: tracks.hasMore, reports };',
      "  await echo.storage.set('lastAudioAuthenticityPage', result);",
      '  return result;',
      '});',
      '',
      ...
      "  const track = input && typeof input === 'object' ? { id: trackId, title: input.title || trackId, artist: input.artist || null } : { id: trackId, title: trackId };",
      '  const result = toDrawerResult(track, report);',
      "  await echo.storage.set('lastAudioAuthenticityTrack', { report, result });",
      '  return result;',
      '});',
    ].join('\n'),
    panel: [
      ...
      '<meta charset="utf-8">',
      '<style>',
      'body{font:13px system-ui;margin:16px;color:#172033;background:#fff}',
      'button{padding:7px 10px;margin-right:8px}',
      'pre{white-space:pre-wrap;background:#f4f6f8;border:1px solid #d8dee7;padding:10px;max-height:360px;overflow:auto}',
      '</style>',
      '<h1>Audio Authenticity</h1>',
      '<button id="run">Analyze page</button>',
      '<button id="next">Next page</button>',
      '<pre id="output">Ready.</pre>',
      '<script>',
      "const pluginId = 'echo.audio-authenticity';",
      "const channel = 'echo:plugin-panel';",
      'let page = 1;',
      'const pending = new Map();',
      "window.addEventListener('message', (event) => {",
      ...
      'const output = document.getElementById("output");',
      'const show = (value) => { output.textContent = JSON.stringify(value, null, 2); };',
      'const analyze = async () => show(await requestHost("plugin:runCommand", { commandId: "analyze-page", args: [page] }));',
      'document.getElementById("run").addEventListener("click", analyze);',
      'document.getElementById("next").addEventListener("click", async () => { page += 1; await analyze(); });',
      '</script>',
    ].join('\n'),
  },
  'source-provider': {
