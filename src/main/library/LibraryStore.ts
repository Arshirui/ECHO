...
export class LibraryStore {
  ...
  locateTrackInTracks(trackId: string, query?: LibraryPageQuery): LibraryTrackLocateResult {
    ...
    if (isNaturalTitleSort(sort)) {
      ...
      absoluteIndex = [...rows]
        .sort((left, right) => compareNaturalTitleRows(left, right) * direction)
        .findIndex((row) => row.id === trackId);
    } else {
      const orderSql = this.unifiedTrackOrderSql(sort, Boolean(searchQuery));
      const positionRow = this.getRow(
        `${unifiedTracksSql},
         ranked_tracks AS (
    ...
    if (absoluteIndex < 0) {
      return {
        found: false,
        reason: this.hasKnownTrackId(trackId) ? 'filtered' : 'not-found',
        track: this.getTrack(trackId),
        items: [],
        page: 1,
        pageSize,
    ...
    const pageResult = this.getTracks({ ...query, page, pageSize, sort: sort as LibraryPageQuery['sort'] });
    const track = pageResult.items.find((item) => item.id === trackId) ?? this.getTrack(trackId);
